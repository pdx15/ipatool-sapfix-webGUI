...
// Purchase history is served by Apple's private "MZPurchaseDaap" service, the
// same DAAP (Digital Audio Access Protocol) endpoint Apple Configurator uses to
// list the apps an Apple ID owns. The flow is three POST requests:
//
//  1. /login                       -> session ID ("mlid")
//  2. /update                      -> current revision ("musr"), SAP-signed
//  3. /databases/{rev}/items       -> DMAP list of owned apps, SAP-signed
//
// Ported from upstream ipatool (majd/ipatool, "feat: add list purchases
// command"). This build signs actions through the platform SAP signer that is
// already wired into the HTTP client (CommerceKit on macOS, sapsigner.exe on
// Windows), so requests simply set SignAction.
...
// OwnedApps lists the apps owned by the authenticated account, newest
// purchase first.
func (t *appstore) OwnedApps(input OwnedAppsInput) (OwnedAppsOutput, error) {
	...
	apps, err := t.fetchOwnedApps(input.Account, guid)
	if err != nil {
		return OwnedAppsOutput{}, err
	}
...
func (t *appstore) ownedAppsUpdateRequest(acc Account, guid string, sessionID uint32, query string) http.Request {
...
func (t *appstore) ownedAppsItemsRequest(acc Account, guid string, sessionID, latestVersion uint32, query string, now time.Time) http.Request {
