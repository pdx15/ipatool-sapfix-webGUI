// Package appleca provides Apple's official root certificates so that TLS
// connections to Apple's authentication and store servers (gsa.apple.com,
// buy.itunes.apple.com, api.apple.com) can be verified on machines whose
// system trust store does not ship the Apple roots.
//
// Windows does not include the Apple root CAs by default, so a stock trust
// store rejects the otherwise-genuine Apple certificate chains with
// "certificate signed by unknown authority". The certificates embedded below
// are Apple's three official root CAs (published at
// https://www.apple.com/certificateauthority/); they are real, trusted public
// anchors, so merging them into the trust pool keeps full TLS verification
// enabled - nothing is bypassed.
package appleca

import (
	"crypto/tls"
	"crypto/x509"
	"fmt"
	"net/http"
)

// Fingerprints (SHA-256) for verification purposes:
//
//	Apple Root CA           b0b1730ecbc7ff4505142c49f1295e6eda6bcaed7e2c68c5be91b5a11001f024
//	Apple Root CA - G2      c2b9b042dd57830e7d117dac55ac8ae19407d38e41d88f3215bc3a890444a050
//	Apple Root CA - G3      63343abfb89a6a03ebb57e9b3f5fa7be7c4f5c756f3017b3a8c488c3653e9179
const (
	rootCAPEM = `-----BEGIN CERTIFICATE-----

UKqK1drk/NAJBzewdXUh
-----END CERTIFICATE-----`

	rootCAG2PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

	rootCAG3PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`
)

// pool returns the system certificate pool augmented with Apple's three
// official root CAs.
func pool() (*x509.CertPool, error) {
	p, err := x509.SystemCertPool()
	if err != nil {
		// No system store available (rare on Windows); start from scratch so
		// the Apple anchors are still honoured.
		p = x509.NewCertPool()
	}

	for _, pem := range []string{rootCAPEM, rootCAG2PEM, rootCAG3PEM} {
		if !p.AppendCertsFromPEM([]byte(pem)) {
			return nil, fmt.Errorf("appleca: failed to append Apple root certificate")
		}
	}

	return p, nil
}

// Pool returns the system certificate pool augmented with Apple's official
// root CAs.
func Pool() (*x509.CertPool, error) {
	return pool()
}

// Transport returns a clone of http.DefaultTransport whose TLS configuration
// verifies against the system store plus Apple's embedded roots. The shared
// http.DefaultTransport is never mutated.
func Transport() (*http.Transport, error) {
	t, ok := http.DefaultTransport.(*http.Transport)
	if !ok {
		t = &http.Transport{}
	}
	clone := t.Clone()

	roots, err := pool()
	if err != nil {
		return nil, err
	}

	clone.TLSClientConfig = &tls.Config{
		RootCAs:   roots,
		MinVersion: tls.VersionTLS12,
	}

	return clone, nil
}
