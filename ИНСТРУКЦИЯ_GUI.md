...
### 7. 📤 Перенос сессий между компьютерами (Экспорт / Импорт)
...
#### Вопрос: Программа падает на Windows 7/8 с ошибкой «Exception 0xc0000005 ... PC=0x0 runtime.asmstdcall». Что делать?
> **Ответ:** Обычный релиз собран стандартным компилятором Go, который начиная с версии 1.21 **требует Windows 10 и новее** — поэтому бинарник падает сразу при запуске с `Exception 0xc0000005 0x8 0x0 0x0 ... runtime/sys_windows_386.s`. Исправляется использованием **legacy-сборки**: она компилируется toolchain'ом [go-legacy-win7](https://github.com/thongtech/go-legacy-win7) (актуальный Go + патчи поддержки Windows 7) через GitHub Actions workflow **«Build Windows 7 (legacy) binaries»** (`.github/workflows/build-windows7.yml`, запускается вручную во вкладке Actions).
>
> Что сделать:
> 1. Скачайте артефакт **`ipatool-webGUI-windows7-386`** и замените им `ipatool.exe` в папке релиза (папку `tools\` не трогайте).
> 2. При входе на Windows дополнительно вызывается `tools\sapsigner.exe` — он в обычном релизе тоже собран под Win10+. Замените его на `sapsigner.exe` из артефакта **`sapsigner-windows7-experimental`** (пересобран из `t0rr3sp3dr0/sapsigner` под Win7) или пропишите путь к нему в переменной `IPATOOL_SAPSIGNER`.
>
> Ограничения:
> - Вспомогательные файлы в `tools\` 64-битные, поэтому для входа нужна **64-битная редакция Windows 7** (на 32-битной Windows вход не работал и в обычном релизе).
> - Утилиты `libimobiledevice` (вкладка «Установка на устройство») под Win7 не пересобираются и могут не заработать; поиск/покупка/скачивание от них не зависят.
> - Если что-то всё равно не заводится — надёжнее всего обновиться до Windows 10/11: Windows 7 не поддерживается с 2020 года.
