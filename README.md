...
> ℹ️ **Откуда берётся `sapsigner.exe` и почему его нет в репозитории.** Этот файл — готовый бинарник, который поставляется вместе с **Signum** (Windows-компаньон AltStore) и находится в `%LOCALAPPDATA%\Signum\resources\apple-tools\windows-x64\v3-legacy\sapsigner.exe`. Он выполняет SAP-подпись, эмулируя проприетарные фреймворки Apple (`CommerceKit`, `CoreFP` из соседней папки `sap-cache`). Мы **не распространяем** его в этом репозитории: это чужой проприетарный бинарник (AltStore + код Apple), и размещение его в репозитории нарушило бы их условия лицензирования. Наша программа просто запускает уже установленный у пользователя `sapsigner.exe` как внешний процесс и находит его автоматически (или по `IPATOOL_SAPSIGNER`).
